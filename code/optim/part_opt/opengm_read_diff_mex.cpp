...
void mexFunction_protect(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if(nrhs != 2) {
		mexErrMsgTxt("Usage: [ee f1 f2] = opengm_read('file_name','gm')\n");
	}
	...
	*/
	...
	for(int loop=0;loop<2;++loop){
				...
				if(!loop){
					...
					for (int k1 = 0; k1 < nL[u]; ++k1){
						for (int k2 = 0; k2 < nL[v]; ++k2){
							f2(k1, k2, e) = value_type(std::min(_f2(k1, k2), double((1 << 29))));
						};
					};
					*/
					int k = 0;
					for (int k1 = 0; k1 < nL[u]; ++k1){
						f2(k, e) = value_type(std::min(_f2(k1, 0), double((1 << 29))));
						++k;
					};
					/*
					for (int k2 = 1; k2 < nL[v]; ++k2){
						f2(k, e) = value_type(std::min(_f2(0, k2), double((1 << 29))));
						++k;
					};
					*/
		...
		if(!loop){
			...
			E.resize(mint2(2,nE));
			//f2.resize(mint2(2*maxL-1, nE));
			f2.resize(mint2(maxL, nE));
			mexPrintf("allocated\n");
		};
	};
