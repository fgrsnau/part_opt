...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	*/
	...
	mx_array<c_type, 2> _f1(prhs[1]);
	mx_array<c_type, 3> _f2(prhs[2]);
	num_array<c_type, 2> f1; f1 = _f1;
	num_array<c_type, 3> f2; f2 = _f2;
	...
	num_array<c_type, 2> g1(mint2(2, nV));
	num_array<c_type, 3> g2(mint3(2, 2, nE));
	...
	for(int e = 0;e<nE;++e){
		...
		for(int j=0;j<K;++j){
			if(j==y(t) || X(j,t)==0)continue;
			c_type r = b - f2(y(s), j, e);
			c_type * p1 = &f2(0, j, e);
			c_type * p2 = &f2(0, y(t), e);
			for(int i=0;i<K;++i){
				if(i==y(s) || X(i,s)==0)continue;
