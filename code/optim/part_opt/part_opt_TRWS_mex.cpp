...
void mexFunction_protect(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	*/
	...
	burn.resize(exttype::mint3(f->maxK,f->nV(),6));
	...
	if (nlhs >= 7){
		mx_array<int, 2> sols;
		sols.resize(mint2(alg->nV, alg->sols.size()));
		for (int i = 0; i < alg->sols.size(); ++i){
			sols.subdim<1>(i) << alg->sols[i];
		};
		plhs[6] = sols.get_mxArray_andDie();
	};
	*/
	delete alg;
	delete f;
