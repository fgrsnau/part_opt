...
__forceinline void stream_store(float * adr, const float & x){
	(*adr) = x;
};
...
__forceinline void stream_store(double * adr, const double & x){
	(*adr) = x;
};
...
__forceinline void stream_store(sse_float_4 * adr, const sse_float_4 & x){
	_mm_stream_ps((float*)adr, x.a);
};
