...
public:
	...
	NEWOPTION(bool, all_to_one, false); // all-to-one class of maps
	...
	NEWOPTION(bool, rebuild_incremental, true);
	/*
	//
	bool use_cut;
	bool use_pixel_cut;
...
public: // interface
	...
	//! run iterations until encounters one of the msg_alg::exit_reason {iterations, precision, target_energy, zero_gap, error}
	virtual void run_converge(double target_E = -std::numeric_limits<double>::infinity()) = 0;
public: //output statistics
	double LB;//!< Lower boiund
	double margin;//!< margin
	dynamic::intf current_x;//!< labeling in the last iteration
	dynamic::intf best_x;//!< best labeling during the run
	double current_E;//!< energy of current_x
	double best_E;//!< energy of best_x
	double dPhi;//!< A measure of message convergence
	int total_it;//!< Total # of iterations performed
	dynamic::num_array<double, 2> hist;//!< History of Lower bound and Energy through iterations
