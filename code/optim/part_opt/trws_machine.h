...
protected:
	...
	struct node_info_core{ // layout: [ node_info_core ] [edge_info_core in_1] ... [edge_info_core in_n] [edge_info_core out_1] [edge_info_core out_n]
		...
		vectorizer * data; // data layout [ f1 ] [ bw ] [ msg_out 1 , data 1] ... [ msg_out n , data n], messages can be of varied size 
		//vectorizer * data_end;
	...
	private:
		...
		vectorizer * _bw(){ return data; };
		vectorizer * const _bw()const{ return data; };
	...
	public:
		...
		vectorizer * const r1()const { return __r1; };
...
protected:
	...
	stream_graph SG;
	size_t total_read; // how much mem needs to be red every inner iteration (statistics)
	size_t total_write; // how much mem needs to be written every inner iteration (statistics)
protected:
	...
	template<bool dir_fw, bool measure> void run_dir();
	template<bool measure> void run_forward_backward();
	void init_iteration();
...
public: // other interface
	virtual void get_M(num_array<double, 2> & M) override; // !Get messages as a single array, M is allocated
	virtual void set_M(const num_array<double, 2> & M) override;// !Set backward messagesas a single array
	void get_message(int e, bool forward, std::vector<type> & x);
public://debug checks
	void check_get_col();
//=======================================================================================
private:
