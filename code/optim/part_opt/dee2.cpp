...
public:
	...
	const num_array<c_type, 2> & f1;
	const num_array<c_type, 3> & f2;
    num_array<int,2> & X;
    num_array<int,2> & P;
    double eps;
public:
    ...
    std::queue<node*> nq;
    std::queue<edge*> eq;
	dynamic::num_array<c_type, 2> delta_s;
	dynamic::num_array<c_type, 2> delta_t;
public:
	...
	dee2_compute(const num_array<int, 2>  & _E, const num_array<c_type, 2> & _f1, const num_array<c_type, 3> & _f2, num_array<int, 2> & _X, num_array<int, 2> & _P, double _eps) :E(_E), f1(_f1), f2(_f2), X(_X), P(_P), eps(_eps) {
        eliminated = 0;
        eliminated2 = 0;
        K = f1.size()[0];
        nV = f1.size()[1];
    ...
    void update_M0(int e) {
        ...
        for(int a = 0; a<K; ++a) { // try eliminate label a
            ...
            for(int b = 0; b<K; ++b) { // by finding a better label b
				...
				const c_type * p1 = &f2(a, 0, e);
				const c_type * p2 = &f2(b, 0, e);
                const int * pX = &X(0,t);
    ...
    void update_M1(int e) {
        ...
        for(int a = 0; a<K; ++a) { // try eliminate label a
            ...
            for(int b = 0; b<K; ++b) { // by finding a better label b
                if(b==a || X(b,s)==0)continue;
				c_type r = INF(c_type);
				const c_type * p1 = &f2(0, a, e);
				const c_type * p2 = &f2(0, b, e);
                const int * pX = &X(0,t);
	...
	void edges_delta(int s, int skip_edge, dynamic::num_array<c_type, 2> & delta) {
        delta << 0;
                    ...
                    for(int a2 = 0; a2<K; ++a2) {
                        ...
                        for(int b1 = 0; b1<K; ++b1) { // by finding a better label (b1,b2)
                            ...
                            for(int b2 = 0; b2<K; ++b2) {
								...
								c_type delta = f1(a1, s) - f1(b1, s) + f1(a2, t) - f1(b2, t) + f2(a1, a2, e) - f2(b1, b2, e);
                                delta += delta_s(a1,b1);
                                delta += delta_t(a2,b2);
...
double dee2(const num_array<int, 2>  & E, const num_array<c_type, 2> & f1, const num_array<c_type, 3> & f2, num_array<int, 2> & X, num_array<int, 2> & P, double eps) {
