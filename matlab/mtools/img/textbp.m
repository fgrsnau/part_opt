function ht = textbp(x0,y0,string, varargin)
...
% do the hard work
pos = tscan(gca,width,height,TOL,[x0 y0]);
% if everything went fine, then put the text onto the plot
if (pos ~= -1)
  ht_local = text(pos(1),1-pos(2),string,'units','normalized',...
...
function [Pos]=tscan(ha,wdt,hgt,tol,target_pos)
...
for i=1:size(Kids),
  ...
  elseif strcmp(get(Kids(i),'type'),'text'),
	...
	warning('Unhandled type of graphical object: %s\n',get(Kids(i),'type'));
  end
  Xdata=[Xdata; Xtemp(:)];
  Ydata=[Ydata; Ytemp(:)];
end
...
% take into account desired location
if (exist('target_pos','var') && ~isempty(target_pos))
	% recalculate target_pos to grid
	sz = size(pop);	
	tx = (target_pos(1)-xlim(1))/(Twdt/2);
	ty = sz(1)-(target_pos(2)-ylim(1))/(Thgt/2)+1;
